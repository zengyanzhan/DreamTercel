...
<script type="text/javascript">
	function quanxianTiaozhuan(roleCode, roleName) {
		...
		location.href = "../getQuanxian.action?roleCode=" + roleCode
				+ "&roleName=" + roleName;
							...
							columns : [
									...
									},
									{
										...
										formatter : function(index, rows) {
											return '<button id="btn_edit" type="button" class="btn-small btn-warning" onclick="quanxianTiaozhuan(\''
													+ rows.roleCode
													+ '\',\''
													+ rows.roleName
													+ '\')">'
				...
				function() {
					...
					$.each(allRows, function(i, oneRow) {
						...
						$
								.ajax({
									type : "post",
									url : "../selectOneRoleByCode.action?roleCode="
											+ oneRow.roleCode,
									//$("#myForm").serializeArray() 以对象的形式得到文本框内的值
...
<body>
	...
	<div id="mianDiv">
		...
		<div id="toolbar" class="panel-heading form-inline">
			...
			&nbsp;&nbsp; 角色姓名:<input class="form-control" id="roleName"
				 /> <label>角色状态：</label>
							...
							<div class="form-inline" style="padding: 10px;">
								<div class="form-group">
									<label for="usrPassword">角色描述:</label> <input type="text"
										...
										placeholder="角色描述" style="width:175px">
								</div>
								&nbsp;&nbsp;
								<div class="form-group">
									<div class="form-group">
										<label for="usrFlag" class="control-label">角色状态:</label> <select
											class="form-control" name="roleFlag"
											data-placement="top" style="width:180px">
											<option value="0">禁用</option>
											<option value="1">启用</option>
										</select>
									</div>
